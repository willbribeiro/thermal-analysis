...
% método Kissinger
for c = 1:length(T_pico)
    T_pico_K(c) = T_pico(c)+273.15;
end
taxa = [5,10,20,40];
for c = 1:length(T_pico_K)
    x_kissinger(c) = 1/T_pico_K(c);
    y_kissinger(c) = log(taxa(c)/(T_pico_K(c)^2));
end
figure (4)
plot(x_kissinger,y_kissinger,'o')
hold on
title('Método de Kissinger')
xlabel('1/Tpico')
ylabel('ln(beta/Tpico^2')
regressao = polyfit(x_kissinger,y_kissinger,1);
plot(x_kissinger,polyval(regressao,x_kissinger),'r-')
Energia_de_ativacao = regressao(1)*8.314